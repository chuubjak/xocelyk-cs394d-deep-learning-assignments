...
# may want to move this inside of the __getitem__ method
# def apply_heatmap(im, dets):
#     hm, size = dense_transforms.detections_to_heatmap(dets, im.shape[1:])
#     hm = hm.numpy().transpose([1, 2, 0])
#     alpha = 0.25*hm.max(axis=2) + 0.75
#     r = np.maximum(hm[:, :, 1], hm[:, :, 2])
#     g = np.maximum(hm[:, :, 0], hm[:, :, 2])
#     b = np.maximum(hm[:, :, 0], hm[:, :, 1])
...
def load_detection_data(dataset_path, num_workers=0, batch_size=32, **kwargs):
    ...
    dataset = [apply_heatmap(im, dets) for im, *dets in dataset]
